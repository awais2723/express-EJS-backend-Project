...
app.post('/add', requireAuth, upload.fields([{ name: 'picture' }, { name: 'brochure' }]), (req, res) => {
...
app.post('/edit/:id', requireAuth, upload.fields([{ name: 'picture' }, { name: 'brochure' }]), (req, res) => {
  const { name, price, availability } = req.body;
  const product = products.find((p) => p.id === parseInt(req.params.id));
  if (!product) return res.status(404).send('Product not found');
  ...
  product.picture = req.files.picture ? req.files.picture[0].filename : product.picture;
  product.brochure = req.files.brochure ? req.files.brochure[0].filename : product.brochure;
